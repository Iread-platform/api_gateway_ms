...
CONTAINER_PORT=$4
HTTPS_CONTAINER_PORT=$5
INTERNAL_PORT=80
HTTPS_INTERNAL_PORT=433
...
echo HTTPS_INTERNAL_PORT=${HTTPS_INTERNAL_PORT}
...
docker run -p 46.227.254.20:${CONTAINER_PORT}:${INTERNAL_PORT} -p 46.227.254.20:${HTTPS_CONTAINER_PORT}:${HTTPS_INTERNAL_PORT} --name ${DOCKER_CONTAINER_NAME} --hostname ${DOCKER_CONTAINER_NAME} -d ${DOCKER_IMAGE_AND_TAG};
